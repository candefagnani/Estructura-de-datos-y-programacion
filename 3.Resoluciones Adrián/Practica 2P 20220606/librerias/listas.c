...
void ordenarLista(Lista *plst)
{
    ...
    if (plst == NULL) {
        ...
        while (actual != NULL) {
            ...
            while (index != NULL) {
                if (actual->d.totalvendido < index->d.totalvendido) {
                    temp = actual->d;
                    actual->d = index->d;
                    index->d = temp;
                }
...
Nodo * obtenerNodo(Lista lst, int productoElegido)
{
    Lista aux;
    aux = lst;
    while(aux != NULL)
    {
        if(aux->d.codigo == productoElegido)
...
{
    printf("Codigo: %d - Nombre: %s - Precio: %d\n",unDato.codigo,unDato.descripcion,unDato.precio);
}

int esIgual(dato datoLista,dato datoAInsertar)
{
    return (datoLista.codigo == datoAInsertar.codigo);
}
